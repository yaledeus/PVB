...
def compute_pairwise_distances(traj, offset=3):
    ...
    for i in range(n_atoms):
        for j in range(i + offset + 1, n_atoms):
            ### pairwise distances should be calculated for the same chain
            if traj.topology.residue(i).chain != traj.topology.residue(j).chain:
                continue
...
def compute_torus(traj):
    """Compute torus of the protein, phi and psi."""
...
def compute_contact_map(traj, contact_threshold=1.0):
    ...
    for i in range(n_residues):
        for j in range(i + 1, n_residues):
            ...
            contact_map[i, j] = contact_rate

    return contact_map, min_dist_matrix
...
def compute_validity(traj, clash_threshold=0.3, bond_break_threshold=0.419) -> object:
    ...
    for i in range(1, num_atoms):
        if traj.topology.residue(i - 1).chain != traj.topology.residue(i).chain:
            adjacent_distances[:, i - 1] = -np.inf
