...
def random_rotation_matrix(angle_limit=np.pi):
...
def compute_crmsd_torch(X, Y, aligned=False):
    if not aligned:
        X_aligned, _, _ = kabsch_torch(X, Y)
    else:
        X_aligned = X
    dist = torch.sum((X_aligned - Y) ** 2, dim=-1)
    crmsd = torch.sqrt(torch.mean(dist))
    return crmsd
