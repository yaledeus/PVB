...
class EquiformerV2_OC20(BaseModel):
    ...
    def __init__(
        ...
        self.block_embedding = nn.Embedding(self.max_num_blocks, self.sphere_channels_all)
        self.t_embedding = nn.Linear(1, self.sphere_channels_all)
        self.bond_embedding = nn.Embedding(2, self.edge_channels)     # 0: no bond, 1: bond
        ...
        self.edge_channels_list = [self.edge_channels * 2] + [self.edge_channels] * 2
    ...
    def forward(
        ...
        edge_distance_vec: Optional[Tensor] = None,
        bond_type: Optional[Tensor] = None
    ):
        self.batch_size = len(z)
        ...
        for i in range(self.num_resolutions):
            if self.num_resolutions == 1:
                x.embedding[:, offset_res, :] = self.sphere_embedding(atomic_numbers) + self.block_embedding(b) + self.t_embedding(t)
            else:
                x.embedding[:, offset_res, :] = (self.sphere_embedding(atomic_numbers) + self.block_embedding(b) + self.t_embedding(t))[:, offset : offset + self.sphere_channels]
        ...
        edge_distance = torch.cat((edge_distance, bond_attr), dim=1)
        if self.share_atom_edge_embedding and self.use_atom_edge_embedding:
        ...
        energy = node_energy.view(-1, self.output_channels)
        # energy = torch.zeros(natoms, device=node_energy.device, dtype=node_energy.dtype)
        ...
        if self.regress_forces:
            ...
            forces = forces.view(-1, 3, self.output_channels)        
