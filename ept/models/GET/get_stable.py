...
@R.register('GETStableEncoder')
class GETStableEncoder(nn.Module):
    def __init__(self, hidden_size, radial_size, n_channel, n_rbf,
        ...
        self.encoder = GETStable(
            hidden_size, radial_size, n_channel, n_rbf,
            d_edge=edge_size, n_layers=n_layers, dropout=dropout,
...
class GETStable(nn.Module):
    '''Equivariant Adaptive Block Transformer'''

    def __init__(self, d_hidden, d_radial, n_channel, n_rbf, cutoff=7.0, d_edge=0, n_layers=4,
                 act_fn=nn.SiLU(), residual=True, dropout=0.1, z_requires_grad=True, use_ffn=True, pre_norm=False,
                 sparsify_k=3):
        super().__init__()
        '''
        ...
        for i in range(0, n_layers):
            self.add_module(f'layer_{i}', GETLayer(
                d_hidden, d_radial, n_channel, n_rbf, cutoff, d_edge, act_fn, residual
            ))
        ...
        if not z_requires_grad:
            i = 1 if self.use_ffn else 0
            self._modules[f'layernorm{i}_{n_layers - 1}'].sigma.requires_grad = False
    ...
    def forward(self, H, Z, block_id, batch_id, edges, edge_attr=None, cached_unit_edge_info=None):
        if cached_unit_edge_info is None:
            with torch.no_grad():
                cached_unit_edge_info = _unit_edges_from_block_edges(block_id, edges.T, Z, self.sparsify_k) # [Eu], Eu = \sum_{i, j \in E} n_i * n_j
        
        # # FFN self-loop
        # self_loop = self.self_loop_edges(block_id, batch_id.shape[0])

        if self.pre_norm:
            H, Z = self.pre_layernorm(H, Z, block_id, batch_id)
...
class GETLayer(nn.Module):
    ...
    def __init__(self, d_hidden, d_radial, n_channel, n_rbf, cutoff=7.0,
                 d_edge=0, act_fn=nn.SiLU(), residual=True):
        ...
        self.att_mlp = nn.Sequential(
            nn.Linear(d_radial * 2 + n_channel * n_rbf + d_edge, d_radial),
            act_fn,
            nn.Linear(d_radial, d_radial),
            act_fn
        )
        self.unit_att_linear = nn.Linear(d_radial, 1)
        self.block_att_linear = nn.Linear(d_radial, 1)
        ...
        self.unit_msg_mlp = nn.Sequential(
            nn.Linear(d_radial + n_channel * n_rbf, d_radial),
            act_fn,
            nn.Linear(d_radial, d_radial),
            act_fn
        )
    ...
    def attention(self, H, Z, edges, edge_attr, cached_unit_edge_info):
        ...
        if self.n_rbf > 1:
            D = self.rbf(D.view(-1)).view(D.shape[0], -1)  # [Eu, n_channel * n_rbf]

        # R = self.reci_sqrt_d * (H_q * H_k).sum(-1) + self.dist_mlp(D).squeeze()   # [Eu]
        if edge_attr is None:
            R_repr = torch.concat([H_q, H_k, D], dim=-1) # [Eu, d_radial * 2 + n_channel * n_rbf]
        else:
            R_repr = torch.concat([H_q, H_k, D, edge_attr[block_edge_id]], dim=-1)
        R_repr = self.att_mlp(R_repr) # [Eu, d_radial]
        R = self.unit_att_linear(R_repr).squeeze() # [Eu]
        ...
        beta = self.block_att_linear(scatter_mean(R_repr, block_edge_id, dim=0)).squeeze() # [Eb]
    ...
    def invariant_update(self, H_v, H, alpha, beta, D, cached_unit_edge_info):
        ...
        H_v = self.unit_msg_mlp(torch.cat([H_v[unit_col], D], dim=-1))  # [Eu, d_radial]
    ...
    def equivariant_update(self, H_v, Z, alpha, beta, D, dZ, cached_unit_edge_info):
        ...
        H_v = self.unit_msg_coord_mlp(torch.cat([H_v[unit_col], D], dim=-1))  # [Eu, n_channel]

        Z_agg = scatter_sum(
            (alpha * H_v).unsqueeze(-1) * dZ,
    ...
    def forward(self, H, Z, block_id, edges, edge_attr=None, cached_unit_edge_info=None):
        ...
        H_v = self.linear_v(H)  # [N, d_radial]
...
class EquivariantFFN(nn.Module):
    ...
    def _radial(self, Z, block_id):
        ...
        D = torch.norm(Z_o + 1e-16, dim=-1)  # [N, n_channel]
...
if __name__ == '__main__':
    ...
    model = GET(d_hidden, d_radial, n_channel, n_rbf, d_edge=d_edge)
    model.to(device)
    model.eval()
