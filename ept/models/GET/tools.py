...
from utils.nn_utils import BatchEdgeConstructor, _knn_edges, _kn1n2_edges, _radial_edges, print_cuda_memory
...
class KN1N2BatchEdgeConstructor(BatchEdgeConstructor):
    def __init__(self, k_neighbors, max_kn1n2, global_message_passing=True, global_node_id_vocab=[], delete_self_loop=True) -> None:
        super().__init__(global_node_id_vocab, delete_self_loop)
        self.k_neighbors = k_neighbors
        self.max_kn1n2 = max_kn1n2
        self.global_message_passing = global_message_passing
    ...
    def _construct_intra_edges(self, S, batch_id, segment_ids, **kwargs):
        ...
        X, block_id = kwargs['X'], kwargs['block_id']
        # block sizes
        block_sizes = scatter_sum(torch.ones_like(block_id), block_id, dim=0)
        ns = block_sizes[all_intra_edges[0]] * block_sizes[all_intra_edges[1]]
        # knn
        src_dst = all_intra_edges.T
        dist = _block_edge_dist(X, block_id, src_dst)
        intra_edges = _kn1n2_edges(
            dist, ns, src_dst, self.k_neighbors, self.max_kn1n2,
            (self.offsets, batch_id, self.max_n, self.gni2lni))
        return intra_edges
    ...
    def _construct_inter_edges(self, S, batch_id, segment_ids, **kwargs):
        ...
        # block sizes
        block_sizes = scatter_sum(torch.ones_like(block_id), block_id, dim=0)
        ns = block_sizes[all_inter_edges[0]] * block_sizes[all_inter_edges[1]]
        # knn
        src_dst = all_inter_edges.T
        dist = _block_edge_dist(X, block_id, src_dst)
        inter_edges = _kn1n2_edges(
            dist, ns, src_dst, self.k_neighbors, self.max_kn1n2,
            (self.offsets, batch_id, self.max_n, self.gni2lni))
        return inter_edges
...
class DiffBatchEdgeConstructor(KNNBatchEdgeConstructor):
    '''
        KNN for intra-edges
        Radial with dynamic cutoff for inter-edges (as supposed by DiffDock and DiffDock-PP)
    '''
    
    def __init__(self, k_neighbors, global_message_passing=True, global_node_id_vocab=[], delete_self_loop=True,
                 cross_cutoff_weight=3, cross_cutoff_bias=20, cross_max_k_neighbors=30) -> None:
        super().__init__(k_neighbors, global_message_passing, global_node_id_vocab, delete_self_loop)
        self.cross_cutoff_weight = cross_cutoff_weight
        self.cross_cutoff_bias = cross_cutoff_bias
        self.cross_max_k_neighbors = cross_max_k_neighbors
        self.tr_s = None

    def _construct_inter_edges(self, S, batch_id, segment_ids, **kwargs):
        '''
            Radial Edge with dynamic distance cutoff
        '''
        dist_cutoff = self.tr_s * self.cross_cutoff_weight + self.cross_cutoff_bias  # [batch_size]
        
        ori_knn = self.k_neighbors
        self.k_neighbors = self.cross_max_k_neighbors
        all_inter_edges = super()._construct_inter_edges(S, batch_id, segment_ids, **kwargs)
        self.k_neighbors = ori_knn
        X, block_id = kwargs['X'], kwargs['block_id']

        # radial cutoff
        src_dst = all_inter_edges.T
        dist = _block_edge_dist(X, block_id, src_dst)
        # normalize dist with cutoff of each data in the batch
        dist_cutoff = dist_cutoff[batch_id[all_inter_edges[0]]]  # [Ef]
        dist = dist / dist_cutoff

        inter_edges = _radial_edges(dist, src_dst, 1)  # radial cutoff can be set to 1 now

        return inter_edges


class DiffSampleEdgeConstructor(KNNBatchEdgeConstructor):
    def __init__(self, k_neighbors, global_message_passing=True, global_node_id_vocab=[], delete_self_loop=True) -> None:
        super().__init__(k_neighbors, global_message_passing, global_node_id_vocab, delete_self_loop)
        self.given_inter_edges = None

    def _construct_inter_edges(self, S, batch_id, segment_ids, **kwargs):
        '''
            Connect farthest point sampling
        '''
        return self.given_inter_edges  # [2, Ef]
...
class BlockEmbedding(nn.Module):
    ...
    '''
    def __init__(self, num_block_type, num_atom_type, num_atom_position, embed_size):
        super().__init__()
