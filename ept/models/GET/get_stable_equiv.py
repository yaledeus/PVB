...
from utils.nn_utils import stable_norm, std_conserve_scatter_sum

from .tools import _unit_edges_from_block_edges
from .radial_basis import RadialBasis


@R.register('GETStableEquivEncoder')
class GETStableEquivEncoder(nn.Module):
    def __init__(self, hidden_size, radial_size, n_channel, n_rbf, cutoff=7.0,
                 edge_size=16, n_layers=3, n_head=4, dropout=0.1,
                 z_requires_grad=True, use_ffn=True, pre_norm=False) -> None:
        super().__init__()

        self.encoder = GETStableEquiv(
            hidden_size, radial_size, n_channel, n_rbf, cutoff=cutoff,
            d_edge=edge_size, n_layers=n_layers, n_head=n_head, dropout=dropout,
...
class GETStableEquiv(nn.Module):
