...
def _kn1n2_edges(dist, ns, src_dst, k_neighbors, max_kn1n2, batch_info):
    '''
    :param dist: [Ef], given distance of edges
    :param ns: [Ef], n1 * n2 of each edge
    :param src_dst: [Ef, 2], full possible edges represented in (src, dst)
    '''
    offsets, batch_id, max_n, gni2lni = batch_info
    ...
    dist_mat = torch.ones(N, max_n, device=dist.device, dtype=dist.dtype) * BIGINT  # [N, max_n]
    n1n2_mat = torch.ones(N, max_n, device=dist.device, dtype=ns.dtype) * BIGINT  # [N, max_n]
    n1n2_mat = n1n2_mat.to(ns.dtype)
    dist_mat[(src_dst[0], gni2lni[src_dst[1]])] = dist
    n1n2_mat[(src_dst[0], gni2lni[src_dst[1]])] = ns
    del dist, ns
    dist_neighbors, dst = torch.topk(dist_mat, k_neighbors, dim=-1, largest=False)  # [N, topk]
    n1n2_mat = torch.stack([n1n2_mat[i][indices] for i, indices in enumerate(dst)]) # [N, topk]
    n1n2_mat = n1n2_mat.cumsum(dim=-1)
    dist_neighbors[n1n2_mat > max_kn1n2] = BIGINT
