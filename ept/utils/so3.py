...
class ApproxAngularDistribution(nn.Module):
    ...
    def __init__(self, stddevs, num_bins=8192, num_iters=1024):
        super().__init__()
    ...
    def _d_pdf(x, e, L):
        ...
        """
        x = x[:, None]  # (N, *)
        c = (1 - torch.cos(x)) + 1e-6 # (N, *)
        l = torch.arange(0, L)[None, :]  # (*, L)
        c1 = torch.sin( (l+0.5)* x ) + 1e-6
        c2 = torch.sin( x / 2 ) + 1e-6
        dc1 = (l+0.5)*torch.cos( (l+0.5)* x ) + 1e-6
        dc2 = 0.5*torch.cos( x / 2 ) + 1e-6
        a = (2*l+1) * torch.exp(-l*(l+1)*(e**2))  # (*, L)
        b = c1 / c2
        db = (dc1 * c2 - dc2 * c1) / (c2 ** 2)
        dc = torch.sin(x) + 1e-6 
        dlogf = (dc / c).reshape(-1) + (a * db).sum(dim = 1) / ((a*b).sum(dim = 1) + 1e-6)
        return dlogf
    ...
    def _precompute_histograms(self):
        ...
        for std in self.stddevs:
            ...
            dy = self._d_pdf(x, std, self.num_iters)
        ...
        self.register_buffer('dY', torch.stack(dY, dim=0))  # (n_stddevs, n_bins)
    ...
    def sample(self, std_idx):
        ...
        d_samples_hist = self.dY[std_idx, bin_idx]
        ...
        # d_samples_gaussian = - std_gaussian.abs()
        ...
        samples = samples_hist
        d_samples = d_samples_hist

        return samples.reshape(size), d_samples.reshape(size)


def random_normal_so3(std_idx, angular_distrib, device='cpu'):
    size = std_idx.size()
    u = F.normalize(torch.randn(list(size)+[3,], device=device), dim=-1)
    theta, d_log_prob = angular_distrib.sample(std_idx)
    w = u * theta[..., None]
    d_log_w = u * d_log_prob[..., None]
    return w, d_log_w